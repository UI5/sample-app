sap.ui.define([
	...
	"sap/m/library",
	"sap/ui/core/Element"
], (Controller, Filter, FilterOperator, JSONModel, Device, formatMessage, mobileLibrary, Element) => {
	"use strict";
	...
	return Controller.extend("sap.ui.demo.todo.controller.App", {
		...
		onInit() {
			...
			if (new Date().getDay() === 5) { // It's Friday! - set button type to "Critical"
				this.ButtonType = mobileLibrary.ButtonType.Critical;
			} else {
				this.ButtonType = mobileLibrary.ButtonType.Default;
			}
		...
		onAfterRendering() {
			const avatarControl = Element.closestTo(document.getElementById(this.createId("avatar-profile")));
			const Helper = sap.ui.require("sap/ui/demo/todo/util/Helper");
			if (avatarControl && Helper) {
				avatarControl.setSrc(Helper.resolvePath('./img/logo_ui5.png'));
			}
		},
		...
		addTodo() {
			...
			const aTodos = this.getTodos().map((oTodo) => {
		...
		onUpdateItemsLeftCount() {
			const iItemsLeft = this.getTodos().filter((oTodo) => {
